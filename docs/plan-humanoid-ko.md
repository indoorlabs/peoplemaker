...
## §13 눈으로 봤다 — 그리고 안 서던 세 원인 (2026-09-14, "진행")

§12 의 "못 본 것"을 봤다. Playwright 로 spacemaker 를 띄워 L자 방에 Humanoid G1
셋을 두고 집기 과제를 돌린 뒤 3D 로 넘겼다 — **처음엔 고리만 있고 몸이 없었다.**
콘솔은 조용했다(`PeopleAgentBodies` 의 `.catch`). 원인은 셋이 겹친 것이었다.

| 원인 | 어떻게 알았나 | 고침 |
| --- | --- | --- |
| 저쪽 잠금 파일이 9월 11일 `2c0cf83` 을 물어 설치본이 `urdf` 를 모른다 | 설치본의 `validateCatalog` 가 `catalog/skeleton` 으로 거부 | `npm update peoplemaker --legacy-peer-deps` → `f53c244` |
| webpack 캐시가 옛 번들을 낸다 (버전 `0.1.0` 그대로) | 번들에 `bodySource` 문자열이 없다 | `.next` 지우고 다시 띄움 |
| `body.glb` 가 없다 — 먼 몸만 복사했는데 몸 세우기는 `catalog.body` 를 받는다 | 404 | 449KB 복사 |

셋을 잡자 브라우저가 카탈로그 · 온몸 · 클립 넷을 받고 로봇 셋이 섰다. 사진은
`Downloads/robot-spacemaker-1..3.png`. `served-packs-bench` 에 둘을 더해 12 ok —
설치본이 카탈로그를 받는가, 온몸 파일이 있는가 (둘 다 일부러 깨서 잡히는 것을 봤다).
인계 문서 1절에 같은 표를 두었다.

**보다가 하나 더.** 되풀이(loop) 30분 판이 5분 22초에 멈췄다 — 과제 하나가
세 번 내리 집기에 실패해 `pick_failed` 로 굳었고, 되풀이는 `done` 만 다시 걸었다.
성공률 0.733 · 상한 3 이면 과제 하나가 그렇게 될 확률이 1.9% 라 긴 판은 거의
늘 일찍 끝난다. 놓친 것은 시도가 그랬던 것이지 과제의 성질이 아니므로, 되풀이
판에서는 `pick_failed` 도 다음 바퀴에 다시 건다. 길이 없거나(aisle_narrow) 손이
안 닿는(too_high) 과제는 그대로 둔다 — 몇 바퀴를 돌아도 같다. 벤치 둘을 더해
31 ok: 성공률 0 되풀이 300초에 pick_failed 19번 · 시도 59 (고침을 되돌리면
1번 · 3 으로 잡힌다), too_high 는 되풀이해도 시도 0.
