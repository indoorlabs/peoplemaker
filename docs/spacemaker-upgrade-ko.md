...
이 문서는 **spacemaker 쪽을 고칠 사람**에게 쓴다. peoplemaker 를 49커밋 뒤의
...
| --- | --- | --- |
| 핀 | `2c0cf83` (2026-09-11) | `HEAD` — 그 뒤 49커밋 |
| `origin/main` | `2c0cf83` | `6578f8b` — **밀었다** (2026-09-14) |
| 서비스하는 팩 | `public/packs/` 3개 · 23MB | 저장소에 14개 · 4.47MB (사람 12 · 로봇 1 · 기준 1) |
...
49커밋이 밀리지 않아 `npm update` 를 해도 받을 것이 없었다. 지금은 밀었으니
...
이 차례가 49커밋 뒤에도 그대로 도는지는 **이쪽 게이트가 매 커밋 증명한다**
...
올릴 자리는 만들어 뒀다 — 위 주소가 공개 Release 다 (팩 14 · 파일 889 ·
175.4MB — 로봇 Unitree G1 도 들어 있다). 받을 때 **파일마다 SHA-256 을 견주므로** 받다 끊긴 GLB 가 이상한
...
**23MB 가 4.47MB 가 되면서 사람이 2명에서 12명이 되고, 로봇(Unitree G1)이 하나 딸려 온다.** 지금 팩의 무게는
...
- **호스팅** — 올려 뒀다. 먼 층은 저장소에 들어 있어 `node_modules` 에 그냥
  오고, 그보다 더 받으려면 공개 Release 를 쓴다:

  ```
  https://github.com/indoorlabs/peoplemaker/releases/tag/packs-2026-09-14
    팩 13 · 파일 878 · 174.7MB
    far 2.1MB · near 43.6MB · all 172.5MB · sheet 174.7MB
  ```

  Release 는 파일 이름에 `/` 를 못 써서 **납작하게** 올라가 있다
  (`rocketbox-f01__clips__idle.glb`). `packs.json` 이 `layout: "flat"` 으로
  그 사실을 말하고, `fetch-packs.mjs` 가 그 규칙으로 주소를 만든다 —
  **받아서 저장하는 모양은 폴더 그대로**라 쓰는 쪽은 몰라도 된다.
