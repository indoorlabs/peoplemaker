...
이 문서는 **spacemaker 쪽을 고칠 사람**에게 쓴다. peoplemaker 를 34커밋 뒤의
...
34커밋이 아직 GitHub 에 없다.

## 지금 어떤 상태인가

| | spacemaker 가 보는 것 | 지금 peoplemaker |
| --- | --- | --- |
| 핀 | `2c0cf83` (2026-09-11) | `HEAD` — 그 뒤 34커밋 |
| `origin/main` | `2c0cf83` | **`2c0cf83`** — 35커밋이 안 밀렸다 |
...
```sh
git push origin main      # peoplemaker · 35커밋
```
...
이 차례가 34커밋 뒤에도 그대로 도는지는 **이쪽 게이트가 매 커밋 증명한다**
(`scripts/check-consumer.mjs`, 검사 34). 저쪽 소스 8개를 읽어 부르는 이름과
차례를 그대로 옮겨 적었고, 일부러 여섯 군데를 깨서 여섯 다 잡히는 것을
...
대부분 텍스처가 붙은 몸째(`body.glb`)와 안 쓰는 클립 여섯이다.
...
## 4. 조심할 것 — **`version` 이 안 바뀐다**

저쪽 팩과 이쪽 팩은 내용이 전혀 다른데 `catalog.json` 의 `version` 이
**둘 다 `0.1.0`** 이고 `builtBy` 도 `peoplemaker/build-pack` 으로 같다.
언제 지은 것인지가 카탈로그에 없다.

그래서 **브라우저나 CDN 이 옛 `catalog.json` 을 물고 있으면 알 길이 없다.**
갈아 끼운 뒤에는 이것으로 확인한다:

```js
const { catalog } = await loadPack({ url: '/packs/rocketbox-f01', GLTFLoader });
catalog.bodyFar?.length   // 2 면 새 팩, undefined 면 옛 팩을 물고 있다
catalog.clips.length      // 3 (저장소 것) 또는 31~34 (받은 것) · 9 면 옛 팩
```

이 구멍은 peoplemaker 쪽에서 메울 일이다 (카탈로그에 지은 때를 적는 것).
아직 안 했다.

## 안 해도 되는 것

- **부르는 차례를 고치는 것** — 위 2·3 말고는 없다. 저쪽이 부르는 아홉
  함수와 다섯 메서드가 그대로 도는 것을 게이트가 검사 40으로 증명한다.
