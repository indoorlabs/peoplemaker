...
runGate('check-pack', (g) => {
  ...
  if (!fullPacks().length) {
    g.skip(`팩을 열어 보는 게이트라 몸이 있어야 한다 — ${HOW_TO_GET_PACKS}`);
    return n;
  }
  ...
  // ── 5. packs/ 의 실제 팩 ──
  {
    ...
    for (const p of packs) {
      ...
      for (const far of Array.isArray(cat.bodyFar) ? cat.bodyFar : []) {
      const farPath = path.join(dir, far.file);
      n++;
      if (!fs.existsSync(farPath)) { g.fail(`far/${p}/${far.ratio}/file`, `${far.file} 이 없다`); continue; }
      const fd = parseGLB(fs.readFileSync(farPath));
      n++;
      if ((fd.json.images || []).length || (fd.json.textures || []).length) {
        g.fail(`far/${p}/${far.ratio}/textures`, '먼 몸에 텍스처가 들어 있다 — 그것을 뺀 것이 이 파일의 요점이다');
      }
      n++;
      ...
      if (j0.join() !== j1.join()) {
        g.fail(`far/${p}/${far.ratio}/joints`, `뼈 차례가 몸과 다르다 (${j1.length} vs ${j0.length}) — 구운 아틀라스가 안 맞는다`);
      }
      n++;
      const pos = fd.json.accessors[fd.json.meshes[0].primitives[0].attributes.POSITION];
      if (pos.count !== far.vertices) {
        g.fail(`far/${p}/${far.ratio}/vertices`, `카탈로그는 정점 ${far.vertices} 인데 파일은 ${pos.count} 이다`);
      }
      n++;
      const idx = fd.json.accessors[fd.json.meshes[0].primitives[0].indices];
      if (idx.count / 3 !== far.triangles) {
        g.fail(`far/${p}/${far.ratio}/triangles`, `카탈로그는 삼각형 ${far.triangles} 인데 파일은 ${idx.count / 3} 이다`);
      }
      n++;
      if (fd.json.meshes[0].primitives[0].attributes.COLOR_0 == null) {
        g.fail(`far/${p}/${far.ratio}/color`, '먼 몸에 정점 색이 없다 — 텍스처도 없으면 회색 덩어리가 된다');
      }
      n++;
